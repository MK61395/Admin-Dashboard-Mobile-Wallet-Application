...
function InvestorDashboard() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:5000/investor")
      .then((response) => {
        ...
        if (editMode) {
          setInvestors(
            investors.map((investor) =>
              investor.investorid === editingId ? response.data : investor
            )
          );
          setEditMode(false);
          setEditingId(null);
  ...
  const handleDelete = (id) => {
    axios
      .delete(`http://localhost:5000/investor/${id}`)
      .then(() => {
        setInvestors(
          investors.filter((investor) => investor.investorid !== id)
        );
      })
      .catch((error) => {
  ...
  return (
    <div className="dashboard-content">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="form-group">
          <label htmlFor="account">Account Status</label>
          <select
            name="account"
